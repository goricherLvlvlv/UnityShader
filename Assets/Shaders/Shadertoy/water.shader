    ...
    SubShader
    {
		Pass{
			...
			float4 frag(v2f i) : SV_TARGET{
				...
				float3 viewDirection = normalize(i.worldVertex - worldPosition);
				...
				float3 color = lerp(getSkyColor(dir), getSeaColor(p, n, light, dir, dist), pow(smoothstep(0.0, -0.05, dir.y), 0.3));
				
				return float4(pow(color, float3(0.75, 0.75, 0.75)), 1.0);
			}			
