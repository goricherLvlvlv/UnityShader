...
Shader "Custom/Chap6/Diffuse Per Vertex"
{
	Properties{
	...
	SubShader
	{
		...
		Pass{
			...
			v2f vert(a2v v){
				...
				// 高光反射(镜面反射) C_light * M_specular * max(0, view•reflection)^Gloss

				// reflectDir = -lightDir + 2 * (lightDir•normalDir) * normalDir
				// fixed3 reflectDir = normalize(reflect(-lightDir, normalDir));
				fixed3 reflectDir = normalize(-lightDir + 2 * dot(lightDir, normalDir) * normalDir);
